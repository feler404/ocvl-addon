...
class OCVLbitwise_andNode(OCVLNode):

    bl_develop_state = DEVELOP_STATE_BETA

    _doc = _("Computes bitwise conjunction of the two arrays (dst = src1 & src2) Calculates the per-element bit-wise conjunction of two arrays or an array and a scalar.")
    ...
    def wrapped_process(self):
        ...
        image_out = self.process_cv(fn=cv2.bitwise_and, kwargs=kwargs)
        self.refresh_output_socket("image_out", image_out, is_uuid_type=True)


def register():
    cv_register_class(OCVLbitwise_andNode)


def unregister():
    cv_unregister_class(OCVLbitwise_andNode)
