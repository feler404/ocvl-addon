...
class OCVLgetTextSizeNode(OCVLNodeBase):
    ...
    baseLine_out = bpy.props.StringProperty(name="baseLine_out", default=str(uuid.uuid4()),
        description="Output parameter - y-coordinate of the baseline relative to the bottom-most text point.")
    retval_out = bpy.props.StringProperty(name="retval_out", default=str(uuid.uuid4()),
        description="Return value.")

    text_in = bpy.props.StringProperty(default="OpenCV", update=update_node,
        description="Text string to be drawn.")
    fontScale_in = bpy.props.IntProperty(default=5, min=1, max=30,update=update_node,
        description="Scale factor that is multiplied by the font-specific base size.")
    fontFace_in = bpy.props.EnumProperty(items=FONT_FACE_ITEMS, default="FONT_HERSHEY_SIMPLEX", update=update_node,
