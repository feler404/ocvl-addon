...
class OCVLconvertMapsNode(OCVLNodeBase):
    ...
    map1_in = bpy.props.StringProperty(name="map1_in", default=str(uuid.uuid4()),
        description="The first input map of type CV_16SC2 , CV_32FC1 , or CV_32FC2 .")
    map2_in = bpy.props.StringProperty(name="map2_in", default=str(uuid.uuid4()),
        description="The second input map of type CV_16UC1 , CV_32FC1 , or none (empty matrix), respectively.")
    dstmap1_out = bpy.props.StringProperty(name="dstmap1_out", default=str(uuid.uuid4()),
        description="The first output map that has the type dstmap1type and the same size as src .")
    dstmap2_out = bpy.props.StringProperty(name="dstmap2_out", default=str(uuid.uuid4()),
        description="The second output map.")

    dstmap1type_in = bpy.props.EnumProperty(items=OUTPUT_MAP_TYPE_ITEMS, default='CV_16SC2', update=update_node,
        description="Type of the first output map that should be.")
    nninterpolation_in = bpy.props.BoolProperty(default=False, update=update_node,
