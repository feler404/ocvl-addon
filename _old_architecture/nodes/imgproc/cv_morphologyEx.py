...
class OCVLmorphologyExNode(OCVLNodeBase):
    ...
    image_in = bpy.props.StringProperty(name="image_in", default=str(uuid.uuid4()),
        description="Source image. The number of channels can be arbitrary. The depth should be one of CV_8U, CV_16U, CV_16S, CV_32F` or ``CV_64F.")
    image_out = bpy.props.StringProperty(name="image_out", default=str(uuid.uuid4()),
        description="Destination image of the same size and type as src .")
    ...
    op_in = bpy.props.EnumProperty(items=MORPH_TYPE_ITEMS, default='MORPH_BLACKHAT', update=update_node,
