...
class OCVLintegral3Node(OCVLNode):
    ...
    sqsum_out = StringProperty(name="sqsum_out", default=str(uuid.uuid4()),
        description=_("integral image for squared pixel values; it is (W+1) x (H+1), double-precision floating-point (64f) array."))
    tilted_out = StringProperty(name="tilted_out", default=str(uuid.uuid4()),
        description=_("Integral for the image rotated by 45 degrees; it is (W+1) x (H+1) array with the same data type as sum."))
...
def register():
    cv_register_class(OCVLintegral3Node)


def unregister():
    cv_unregister_class(OCVLintegral3Node)
