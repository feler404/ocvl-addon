...
class OCVLundistortNode(OCVLNodeBase):
    ...
    newCameraMatrix_in = bpy.props.StringProperty(name="newCameraMatrix_in", default=str(uuid.uuid4()),
        description="Camera matrix of the distorted image. By default, it is the same as cameraMatrix but you may additionally scale and shift the result by using a different matrix.")

    image_out = bpy.props.StringProperty(name="image_out", default=str(uuid.uuid4()),
        description="Output (corrected) image.")
