...
class OCVLmomentsNode(OCVLNodeBase):
    ...
    moments_out = bpy.props.StringProperty(name="moments_out", default=str(uuid.uuid4()),
        description="Output moments.")
    image_in = bpy.props.StringProperty(name="image_in", default=str(uuid.uuid4()),
        description="Raster image (single-channel, 8-bit or floating-point 2D array) or an array") #pobrane z opisu dla 'array' nie wiem czy dobrze
    binaryImage_in = bpy.props.BoolProperty(default=False, update=update_node,
