...
class OCVLgetDefaultNewCameraMatrixNode(OCVLNodeBase):
    ...
    cameraMatrix_in = bpy.props.StringProperty(default=str(uuid.uuid4()),
        description="Input camera matrix.")
    imgsize_in = bpy.props.IntVectorProperty(default=(100, 100), min=1, max=2048, size=2, update=update_node,
        description="Camera view image size in pixels.")
    centerPrincipalPoint_in = bpy.props.BoolProperty(default=False, update=update_node,
        description="Location of the principal point in the new camera matrix. The parameter indicates whether this location should be at the image center or not.")
