...
class OCVLGFTTDetectorNode(OCVLFeature2DNode):
    ...
    _doc = _("Wrapping class for feature detection using the goodFeaturesToTrack function.")
    ...
    loc_work_mode = EnumProperty(items=GFTT_WORK_MODE_ITEMS, default="DETECT", update=update_layout, description=_(""))
    ...
    maxCorners_init = IntProperty(default=1000, min=10, max=10000, update=update_and_init, description="")
    qualityLevel_init = FloatProperty(default=0.01, min=0.001, max=0.99, update=update_and_init, description="")
    minDistance_init = FloatProperty(default=1., min=1.0, max=3., update=update_and_init, description="")
    blockSize_init = IntProperty(default=3, min=1, max=8, update=update_and_init, description="")
    useHarrisDetector_init = BoolProperty(default=False, update=update_and_init, description="")
    k_init = FloatProperty(default=0.04, min=0.01, max=0.1, update=update_and_init, description="")
    gradiantSize_init = IntProperty(default=1, min=0, max=100, update=update_and_init, description="")
...
def register():
    cv_register_class(OCVLGFTTDetectorNode)


def unregister():
    cv_unregister_class(OCVLGFTTDetectorNode)
