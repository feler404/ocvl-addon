...
class OCVLfastNlMeansDenoisingColoredNode(OCVLNodeBase):
    ...
    h_in: bpy.props.IntProperty(name="h_in", default=10, min=1, max=128, update=update_node, description="Parameter regulating filter strength for luminance component. Bigger h value perfectly removes noise but also removes image details, smaller h value preserves details but also preserves some noise.")
    hColor_in: bpy.props.IntProperty(name="hForColorComponents_in", default=10, min=1, max=128, update=update_node, description="The same as h but for color components. For most images value equals 10 will be enought to remove colored noise and do not distort colors.")
    ...
    def init(self, context):
        ...
        self.inputs.new("OCVLObjectSocket", "hColor_in").prop_name = "hColor_in"
    ...
    def wrapped_process(self):
        kwargs = {
            ...
            'hColor_in': self.get_from_props("hColor_in"),
        }

        dst_out = self.process_cv(fn=cv2.fastNlMeansDenoisingColored, kwargs=kwargs)
