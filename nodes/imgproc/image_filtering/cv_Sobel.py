...
class OCVLSobelNode(OCVLNodeBase):
    ...
    dx_in: bpy.props.IntProperty(default=1, min=0, max=32, update=update_node, set=set_dx, get=get_dx, description="Order of the derivative x.")
    dy_in: bpy.props.IntProperty(default=1, min=0, max=32, update=update_node, set=set_dy, get=get_dy, description="Order of the derivative y.")
    ...
    ksize_in: bpy.props.IntProperty(default=3, update=update_node, min=1, max=32, set=set_ksize, get=get_ksize, description="Aperture size used to compute the second-derivative filters.")
    scale_in: bpy.props.FloatProperty(default=1.0, min=1, max=32, update=update_node, description="Optional scale factor for the computed Laplacian values.")
    ...
    def wrapped_process(self):
        ...
        dst_out = self.process_cv(fn=cv2.Sobel, kwargs=kwargs)
