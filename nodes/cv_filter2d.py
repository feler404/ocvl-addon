...
class OCVLfilter2dNode(OCVLNodeBase):
    ...
    _doc = "Convolves an image with the kernel."
    ...
    kernel_size_in = IntVectorProperty(default=(1, 1), update=update_node, min=1, max=30, size=2, description="Convolution kernel (or rather a correlation kernel), a single-channel floating point matrix; if you want to apply different kernels to different channels, split the image into separate color planes using split and process them individually.")
    ddepth_in = EnumProperty(items=COLOR_DEPTH_ITEMS, default='CV_8U', update=update_node, description="Desired depth of the destination image, see @ref filter_depths 'combinations'")
    anchor_in = IntVectorProperty(default=(-1, -1), update=update_node, get=get_anchor, set=set_anchor, size=2, description="Anchor of the kernel that indicates the relative position of a filtered point within the kernel; the anchor should lie within the kernel; default value (-1,-1) means that the anchor is at the kernel center.")
    delta_in = IntProperty(default=0, update=update_node, min=0, max=255, description="Optional value added to the filtered pixels before storing them in dst.")
